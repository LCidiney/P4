...
function gerarQuestoes() {
    ...
    dadosProva.assuntos.forEach((assunto) => {
        ...
        for (let i = 0; i < assunto.questoes; i++) {
...
document.querySelector('.avancar').addEventListener('click', function(e) {
    ...
    if (!isValid) {
        alert('Por favor, preencha todos os campos obrigatórios e selecione as alternativas corretas antes de prosseguir.');
    ...
    const provas = JSON.parse(localStorage.getItem("provas") || '[]');
    if (provas.length === 0) {
        alert('Erro: Prova não encontrada');
        window.location.href = '../../index.html';
        return;
    }
